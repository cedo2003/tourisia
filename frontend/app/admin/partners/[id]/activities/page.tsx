...
export default function PartnerActivities() {
    ...
    const fetchActivities = async () => {
        setIsLoading(true);
        try {
            const res = await fetch(`http://localhost:8000/backend/admin/get_partner_activities.php?partner_id=${id}`);
                ...
                <div className="mx-auto max-w-6xl">
                    ...
                    <div className="mb-8 ">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                            <div>
                                ...
                                <p className="text-muted-foreground">Monitoring des activités et performances en temps réel.</p>
                            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                        <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <p className="text-xs font-bold uppercase text-muted-foreground">Vues totales</p>
                            </div>
                            <p className="text-2xl font-bold">{activities.stats.total_views}</p>
                        ...
                        <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
                            ...
                            <p className="text-2xl font-bold">{activities.stats.total_bookings}</p>
                        ...
                        <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <p className="text-xs font-bold uppercase text-muted-foreground">Chiffre d'affaires</p>
                            </div>
                        ...
                        <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <p className="text-xs font-bold uppercase text-muted-foreground">Offres actives</p>
                            </div>
                            <p className="text-2xl font-bold">{activities.stats.active_offers}</p>
                            <p className="mt-2 text-xs text-muted-foreground">En ligne actuellement</p>
                        </div>
                    </div>
                    ...
                    <section className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden mb-10">
                        <div className="p-6 border-b border-border flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">Historique des interactions récentes du partenaire.</p>
                            </div>
                            <button className="text-xs text-[#2563eb] font-bold hover:underline">Voir tout l'historique</button>
                        </div>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">
                                                {new Date(action.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                            </span>
                                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 bg-card p-8 rounded-2xl border border-border shadow-sm">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm">
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Responsable</p>
                                        <p className="font-medium">{partner.manager_name}</p>
                                        <p className="text-muted-foreground text-xs">{partner.manager_email}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Localisation</p>
                                        <p className="text-muted-foreground">{partner.city}, {partner.country}</p>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Date d'inscription</p>
                                        <p className="text-muted-foreground">{new Date(partner.created_at).toLocaleDateString()}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase mb-1">Type d'activité</p>
                        ...
                        <div className="bg-gradient-to-br from-[#2563eb] to-[#123e9c] p-8 rounded-2xl text-white shadow-xl shadow-blue-500/20 flex flex-col justify-between">
                            <div>
                                <p className="text-xs font-bold uppercase opacity-80 mb-2">Abonnement</p>
                                <h3 className="text-2xl font-bold mb-4">{partner.selected_plan}</h3>
