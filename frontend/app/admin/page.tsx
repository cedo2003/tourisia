...
export default function AdminDashboard() {
    const [stats, setStats] = useState({ users: 0, partners: 0, pending_partners: 0 });
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const partnersRes = await fetch("http://localhost:8000/backend/admin/get_partners.php");
            const partnersData = await partnersRes.json();
            setPendingPartners(partnersData.filter((p: any) => Number(p.validation) === 0));
        } catch (err) {
    ...
    const updatePartnerStatus = async (partnerId: string, status: number) => {
        try {
            const res = await fetch("http://localhost:8000/backend/admin/update_partner_status.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ partner_id: partnerId, status })
            });
            if (res.ok) {
                toast.success(status === 1 ? "Partenaire validé !" : "Partenaire refusé.");
                setShowModal(false);
                fetchData();
            }
        } catch (err) {
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-8">
                <div className="mx-auto max-w-6xl">
                    <header className="mb-10">
                        ...
                        <p className="mt-2 text-muted-foreground">Résumé des performances et validations en attente.</p>
                    </header>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-3 mb-10">
                        ...
                        <div className="rounded-2xl bg-card p-6 shadow-sm border border-border">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <Link href="/admin/partners" className="text-muted-foreground hover:text-foreground transition-colors">
                                    <ArrowUpRight className="h-5 w-5" />
                                </Link>
                            </div>
                            <p className="text-sm font-medium text-muted-foreground">Partenaires Validés</p>
                            <p className="text-2xl font-bold">{stats.partners - stats.pending_partners}</p>
                        </div>
                    ...
                    <section className="rounded-2xl bg-card shadow-sm border border-border overflow-hidden">
                        <div className="p-6 border-b border-border">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">Nouveaux partenaires à valider pour accès pro.</p>
                        </div>
                                ...
                                <tbody className="divide-y divide-border">
                                    {pendingPartners.length === 0 ? (
                                        ...
                                        pendingPartners.map((partner: any) => (
                                            <tr key={partner.id} className="text-sm hover:bg-muted/30 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="font-medium">{partner.business_name}</div>
                                                    <div className="text-xs text-muted-foreground">{partner.city}, {partner.country}</div>
                                                </td>
                                                <td className="px-6 py-4 capitalize text-muted-foreground">{partner.activity_type}</td>
                    ...
                    <div className="w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl bg-card shadow-2xl border border-border animate-in zoom-in-95 duration-200">
                        <div className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-border bg-card/95 backdrop-blur-sm">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold">{selectedPartner.business_name}</h2>
                                    <p className="text-sm text-muted-foreground capitalize">{selectedPartner.activity_type} • En attente de validation</p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="p-8 space-y-12">
                                    ...
                                    <div className="space-y-4 text-sm">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-muted-foreground">{selectedPartner.address || "N/A"}, {selectedPartner.city}, {selectedPartner.country}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-muted-foreground">{selectedPartner.business_phone || "N/A"}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-muted-foreground">{selectedPartner.business_email || "N/A"}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-muted-foreground text-[#2563eb]">{selectedPartner.website || "Non renseigné"}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-4 text-sm">
                                        <div className="bg-muted/30 p-4 rounded-xl border border-border">
                                            <p className="font-bold text-foreground text-base">{selectedPartner.manager_name}</p>
                                            <p className="text-muted-foreground">{selectedPartner.manager_role}</p>
                                            <div className="grid grid-cols-1 gap-2 mt-3">
                                                <p className="text-xs flex items-center gap-2 text-muted-foreground"><Mail className="h-3 w-3" /> {selectedPartner.manager_email}</p>
                                                <p className="text-xs flex items-center gap-2 text-muted-foreground"><Phone className="h-3 w-3" /> {selectedPartner.manager_phone}</p>
                                            </div>
                                        </div>
                                        <div className="pt-2">
                                            <p className="text-xs font-bold uppercase text-muted-foreground mb-1">Plan choisi</p>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-3">
                                        <p className="text-sm font-bold text-muted-foreground">Document d'identité</p>
                                            ...
                                            {selectedPartner.identity_document ? (
                                                <>
                                                    {isPdf(selectedPartner.identity_document) ? (
                                                        <iframe
                                                            src={getFileUrl(selectedPartner.identity_document)}
                                                ...
                                                <span className="text-xs text-muted-foreground italic">Aucun document</span>
                                            )}
                                        </div>
                                        <a href={getFileUrl(selectedPartner.identity_document)} target="_blank" rel="noreferrer" className="text-xs text-[#2563eb] hover:underline flex items-center gap-1">
                                    ...
                                    <div className="space-y-3">
                                        <p className="text-sm font-bold text-muted-foreground">Certificat d'existence</p>
                                        <div className="relative aspect-video rounded-xl border-2 border-dashed border-border overflow-hidden bg-muted/20 flex items-center justify-center">
                                            {selectedPartner.existence_certificate ? (
                                                <>
                                                    {isPdf(selectedPartner.existence_certificate) ? (
                                                        <iframe
                                                            src={getFileUrl(selectedPartner.existence_certificate)}
                                                            className="w-full h-full border-none"
                                                            title="Cert PDF"
                                                        />
                                                    ) : (
                                        ...
                                        <a href={getFileUrl(selectedPartner.existence_certificate)} target="_blank" rel="noreferrer" className="text-xs text-[#2563eb] hover:underline flex items-center gap-1">
                                            Ouvrir en plein écran
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 rounded-2xl bg-muted/10 border border-border text-sm">
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase">IBAN / Compte</p>
                                        <p className="font-medium mt-1">{selectedPartner.iban || "N/A"}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase">Banque</p>
                                        <p className="font-medium mt-1">{selectedPartner.bank_name || "N/A"}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-muted-foreground uppercase">Mobile Money</p>
                                        <p className="font-medium mt-1">{selectedPartner.mobile_money_number || "N/A"}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
