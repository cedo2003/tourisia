...
export default function AdminUsers() {
    ...
    const toggleUserStatus = async (userId: string, currentStatus: any) => {
        ...
        try {
            const res = await fetch("http://localhost:8000/backend/admin/update_user_status.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_id: userId, is_active: !currentStatus })
            });
            if (res.ok) {
                toast.success(!currentStatus ? "Compte réactivé." : "Compte désactivé.");
                fetchUsers();
            } else {
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-8">
                <div className="mx-auto max-w-6xl">
                    <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-foreground">Gestion Voyageurs</h1>
                            <p className="mt-2 text-muted-foreground">Supervision des comptes utilisateurs et actions de modération.</p>
                        </div>
                                            ...
                                            <tr key={user.id} className="text-sm hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <div className={`h-2 w-2 rounded-full ${Number(user.is_active) ? "bg-green-500" : "bg-red-500"}`} />
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button
                                                            onClick={() => toggleUserStatus(user.id, Number(user.is_active))}
                                                            ...
                                                            title={Number(user.is_active) ? "Désactiver le compte" : "Activer le compte"}
                                                        >
                                                            {isActionLoading === user.id ? <Loader2 className="h-4 w-4 animate-spin" /> : (Number(user.is_active) ? <UserX className="h-4 w-4" /> : <UserCheck className="h-4 w-4" />)}
                                                        </button>
                                                        ...
                                                        <AlertDialog>
                                                            ...
                                                            <AlertDialogContent>
                                                                <AlertDialogHeader>
                                                                    ...
                                                                    <AlertDialogDescription>
                                                                        Voulez-vous vraiment supprimer le compte de <strong>{user.fullname}</strong> ?
