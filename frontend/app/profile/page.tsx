...
const settingsSections = [
    { icon: User, label: "Informations personnelles", desc: "Modifiez votre nom, email et photo de profil" },
    { icon: Shield, label: "Sécurité", desc: "Mot de passe, authentification à deux facteurs" },
    { icon: Bell, label: "Notifications", desc: "Préférences pour les emails et notifications push" },
    { icon: CreditCard, label: "Moyens de paiement", desc: "Gérez vos cartes et facturation" },
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:8000/backend/profile/get_profile.php?id=${userData.id}`)
                if (response.ok) {
    ...
    const handleSaveBio = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8000/backend/profile/update_profile.php", {
                method: "POST",
            ...
            if (response.ok) {
                ...
                localStorage.setItem("user", JSON.stringify({ ...user, bio: editedBio }))
                setIsEditing(false)
            } else {
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success("Profil mis à jour !")
                const updatedUser = { ...user, fullname: editName, phone: editPhone, location: editLocation }
                setUser(updatedUser)
    ...
    const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: 'avatar' | 'cover_image') => {
        ...
        try {
            const response = await fetch("http://localhost:8000/backend/profile/upload_image.php", {
                method: "POST",
                body: formData
            })
    ...
    // Date formatting
    const memberSinceDate = user.created_at ? new Date(user.created_at).toLocaleDateString('fr-FR', {
        month: 'long',
        year: 'numeric'
    }) : "Inconnu"
                        ...
                        <div className="relative flex flex-col items-start gap-5 pb-6 sm:flex-row sm:items-end sm:gap-6">
                                ...
                                <div className="flex h-28 w-28 items-center justify-center rounded-2xl border-4 border-card bg-[#2563eb] shadow-lg sm:h-36 sm:w-36 overflow-hidden">
                                    {user.avatar ? (
                                        ...
                                        <span className="text-3xl font-bold text-white sm:text-4xl text-center uppercase">
                                            {user.fullname ? user.fullname.split(' ').map((n: string) => n[0]).join('').slice(0, 2) : "SJ"}
                                        </span>
                                    )}
                                </div>
                                <input
                            ...
                            <div className="flex-1 pb-1">
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {user.verified && (
                                        <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#2563eb]" title="Vérifié">
                ...
                <section className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
                    ...
                    {activeTab === "overview" && (
                        <div className="grid gap-6 lg:grid-cols-3">
                            ...
                            <div className="flex flex-col gap-6 lg:col-span-2">
                                ...
                                <div className="rounded-2xl border border-border bg-card p-6">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-lg font-semibold text-foreground">Voyages à venir</h2>
                                        <button
                                ...
                                <div className="rounded-2xl border border-border bg-card p-6">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-lg font-semibold text-foreground">Voyages récents</h2>
                                        <button
                                            onClick={() => setActiveTab("trips")}
                                            className="flex items-center gap-1 text-xs font-medium text-[#2563eb] hover:text-[#1d4ed8]"
                                        >
                                            ...
                                            >
                                                <div className="relative aspect-[16/10] overflow-hidden">
                                                    ...
                                                    <div className="absolute bottom-3 left-3">
                                                        <h3 className="text-sm font-semibold text-white">{trip.destination}</h3>
                                                ...
                                                <div className="flex items-center justify-between p-3">
                                                    ...
                                                    {trip.reviewed ? (
                                                        <span className="text-xs text-emerald-600 font-medium">Avis publié</span>
                                                    ) : (
                            ...
                            <div className="flex flex-col gap-6">
                                ...
                                <div className="rounded-2xl border border-border bg-card p-6">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-lg font-semibold text-foreground">Favoris</h2>
                                        <button
                                ...
                                <div className="rounded-2xl border border-border bg-card p-6">
                                    <h2 className="text-lg font-semibold text-foreground">Carte de mes voyages</h2>
                                    <div className="mt-4 flex aspect-square items-center justify-center rounded-xl bg-muted">
                                        <div className="text-center">
                                            ...
                                            <p className="mt-2 text-sm font-medium text-muted-foreground">28 pays visités</p>
                                            <p className="text-xs text-muted-foreground">Continuez à explorer pour remplir la carte !</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === "trips" && (
                        <div className="flex flex-col gap-8">
                            {/* Voyages à venir */}
                            <div>
                                <h2 className="text-xl font-bold text-foreground">Voyages à venir</h2>
                                <p className="mt-1 text-sm text-muted-foreground">Vos réservations confirmées et en attente.</p>
                                <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    ...
                                    >
                                        ...
                                        <p className="mt-3 text-sm font-semibold text-foreground">Planifier un nouveau voyage</p>
                                        <p className="mt-1 text-xs text-muted-foreground">Parcourir les destinations</p>
                                    </Link>
                                </div>
                            </div>
                            ...
                            {/* Voyages passés */}
                            <div>
                                <h2 className="text-xl font-bold text-foreground">Voyages passés</h2>
                                <p className="mt-1 text-sm text-muted-foreground">Vos souvenirs de voyage et avis.</p>
                                    ...
                                    .map((trip) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                                                    Une expérience absolument merveilleuse ! Le service était impeccable, l’emplacement à couper le souffle, et je le recommande à tous ceux qui cherchent un voyage inoubliable. Je reviendrai sans hésiter.
                                                </p>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-in fade-in zoom-in duration-200">
                        <div className="flex items-center justify-between border-b border-border p-6">
                            <h2 className="text-xl font-bold text-foreground">Modifier le profil</h2>
                            <button onClick={() => setShowEditModal(false)} className="rounded-full p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                                <X className="h-6 w-6" />
                            </button>
                        </div>
                        <div className="p-6">
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-muted-foreground mb-1">Email (non modifiable)</label>
                                    <input
                                        type="email"
                                        disabled
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-muted-foreground mb-1">Localisation</label>
                                    <input
                                        type="text"
                                        className="w-full rounded-lg border border-border bg-background p-2.5 text-sm text-foreground focus:border-[#2563eb] focus:outline-none"
                                        value={editLocation}
